...
import {HLEALM} from "../modules/HLEALM.sol";
...
contract TestableHLEALM is HLEALM {
    ...
    function forceInitialize(uint256 initialPrice) external {
        ...
        state.fastVar = 0;
        state.slowVar = 0;
        // Set alpha values (required for variance calculations)
        state.fastAlpha = 1e17;  // 0.1 in WAD
        state.slowAlpha = 1e16;  // 0.01 in WAD
    }
